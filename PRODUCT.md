# Cairn 产品定义文档（PRODUCT.md v3）

> 版本：v3.0（Product Surface reframe）
> 日期：2026-05-08
> 状态：Product MVP 阶段（kernel 工程已交付，转向 desktop side panel 产品形态）
> 文档定位：v3 是 v2 的**产品面 reframe**——kernel layer 完整保留为底层，user-facing 框架升级为 project-scoped agent work side panel。本文档 §0~§13 是 v3 主体；§14~§17 沿用 v2 工程事实；§18 是 v1 历史归档；§19 是变更记录。

---

## 0. TL;DR（30 秒读完）

Cairn 是 **AI 编程项目的本机 project control surface**——project-scoped agent work side panel。

Cursor / Codex / Claude Code / Kiro 是 **coding surfaces**——你在它们里面写代码、和 agent 对话。**Cairn 不是 coding surface。** 它是它们旁边的**项目现场控制层**。
...
**Cairn 让程序员在长程多 agent 编程任务里不丢掉项目掌控感。** 代码执行交给 Codex / Claude Code / Cursor / Kiro；Cairn 只做：让你看见、让你接得上、让你能回退。

**形态**：本机桌面侧边窗 + tray status icon + ambient floating marker + Live Run Log。极简、高信息密度。形态参考 Activity Monitor、Windows 任务管理器、journalctl。**不**参考 Cursor / Jira / Linear / Asana。
...
### 1.2 四层架构（不是版本路线，是同时存在的层）

Cairn 现在 = 4 层叠加，不是 4 个 phase。每层都可独立讨论，但只有 4 层一起，才是 Cairn。

| 层 | 描述 | 用户感知 | 当前状态 |
|---|---|---|---|
| **Product layer** | project-scoped agent work side panel / project control surface | 用户看到的产品 | Product MVP 待建（§6.2） |
...
| **Storage layer** | SQLite + 8 host-level state objects + future events log | 持久化基础 | ✅ 10 migrations 已交付 |

关键澄清（避免再次形态混淆）：

- **MCP 是 agent 接入 Cairn 的方式，不是 Cairn 的产品形态。** 用户不通过 MCP 用 Cairn；agent 通过 MCP 把工作现场告诉 Cairn。
- **desktop side panel / tray / floating marker / Live Run Log 才是用户感知的产品形态。**
- 4 层之间是从下到上的支撑关系：Storage → Kernel → Integration → Product。任一层缺位，上层失效。
...
4. **不是 lead-subagent orchestrator**——不拆任务、不派 agent、不替 agent reasoning、不替用户拍板
5. **不是 Jira / Linear / Asana / sprint planning / gantt / 企业 PM SaaS**——没有 sprint / story point / 看板 / 资源分配 / burn-down；按真实 agent 工作现场组织，不按虚构的"任务条目"
6. **不是 generic agent framework / SDK**——面向最终用户的产品，不是给开发者造 agent 用的库
7. **不是 plain MCP service**——MCP 是接入方式，不是产品形态本身
8. **不做跨机协作 / 多人共享 daemon**——本机单用户优先，跨机是单独的产品方向
9. **不是 v1 那种"用户的 agent 化身"**——floating marker 是 ambient 状态显示（schema → sprite 动画契约 §8.5），不接受开发对话、不替 agent 写代码

**Cairn 是**：

1. **project control surface**——按本机 project 组织 agent 工作现场（主定位词）
2. **project-scoped agent work side panel**——桌面侧边窗、tray icon、ambient floating marker
3. **coordination kernel window**——通过 read-only Live Run Log 把 kernel 的 8 类状态对象暴露给程序员
4. **run log / recovery surface**——可复盘、可接力、可回退
5. **AI PMO layer**（辅助 framing，介绍场景用，不作为主定位词）——*layer*，不是 *tool* / *suite* / *clone*。详 §15.4 与 Linear / Jira 的硬区分。
...
两类用户在 v2 已锁定，v3 继续覆盖。

### 3.3 明确排除的用户

- **单 agent 用户**：只用一个工具，问题尚未出现，Cairn 加不了价值
- **非技术终端用户**：MVP 假设用户能读 git / 配置 MCP / 理解 state machine
...
**D9. Product MVP read-only 边界（v3 新增）**

决议：
- Product MVP side panel **不**支持 mutation：不答 blocker / 不 rewind / 不 dispatch / 不 resolve conflict
- 所有 mutation 仍走 agent / CLI / MCP 路径
- panel 唯一允许的"动作"是 preview（rewind preview / outcomes evaluate preview）和触发外部跳转（"复制 ckpt-id" 类）

**为什么严格 read-only**（防御"为什么 panel 看到 blocker 不能直接答"的合理质疑）：

1. **可信可见性必须先做透**——panel 第一次出现，用户对它的信任要先建立在"显示是对的"上，再加"动是对的"。先做可信可见性，再评估写操作。
2. **写操作有三个前置条件未满足**：
   - **Daemon 独立 API**：当前 mcp-server 直接 import `daemon/dist/`，agent 是写者；panel 走另一条路径写需要先把 daemon 独立成进程并定义稳定 IPC API（详 §9.7 Later 架构）
   - **Supervisor identity 模型**：panel 是 user-supervisor 触发的写，与 agent 触发的写在 audit trail 里必须可区分（who / why / via）；当前 SESSION_AGENT_ID 只覆盖 agent 侧，supervisor 侧需要新模型
   - **Dogfood 信号支持**：哪种 mutation 真的能省事（一键答 blocker？preview 后直接 rewind？）需要 dogfood ≥ 3 天后识别；先猜没意义
3. **三条都满足后再做 mutation**——这是 D9 升级路径，不是 D9 撤回。Later 段对应项目（answer blocker from desktop / rewind from desktop / resolve conflict from desktop）的触发条件就是上述三条全亮。
...
> 本文是 Cairn 的定义性文档 v3。与本文冲突的任何 plan / note / 旧稿（包括 v1 PRODUCT.md）都以本文为准，除非经过显式的变更记录（§19）更新。
>
> **v3 reframe 的核心断言**：kernel layer（v2 论题）+ product layer（v3 论题）平行存在。已交付的 28 工具 / 8 host-level state objects / 10 migrations 不被 reframe 贬值，是 v3 的 supporting evidence。
