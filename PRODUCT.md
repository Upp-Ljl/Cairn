...
3. **Cairn 不是 v1 那种"用户的 agent 化身"**——不接开发需求、不写代码、不替你跟 agent 对话。v0.1 主交互层是 CLI + MCP 工具；`packages/desktop-shell/` 的 Electron 悬浮宠物是 ambient 状态显示（schema 状态 → sprite 动画契约），不接受开发对话，详 §8.1 / §8.2。
...
- 消息可达的反汇总和不一致检测
- Inspector 主视图 = **Live Run Log**（read-only / event-oriented kernel window；详 §10.3 框架决议 + `docs/superpowers/plans/2026-05-29-v0.2-live-run-log.md`）
...
- 桌面悬浮标（Floating Marker）+ Inspector panel UI（Electron 实施，详 §8.2）。v0.2 主要工作之一。
- L3~L5 checkpoint（对话、工具调用 trace、agent 内部态）
- 冲突检测完整版 + 红线系统
- Dispatch 自动 agent 选型
- **Inspector 主视图 = Live Run Log**（read-only / event-oriented kernel window；详 `docs/superpowers/plans/2026-05-29-v0.2-live-run-log.md`）

**Inspector 主视图框架决议（2026-05-29 锁）**：v0.2 Inspector 不做 Jira-style status dashboard，做 **Live Run Log**——一个 read-only、事件驱动的 host-level coordination kernel 窗口，呈现 agents / subagents / tasks / blockers / outcomes / checkpoints / conflicts 随时间流动的事件，让 supervisor 能复盘和监督多 agent 协作现场。kernel 的 UI 形态从来不是 dashboard，是 *window into itself*（Linux `/proc`、dtrace、journalctl 一脉）。这条决议 veto 任何让 Inspector 漂移成 "observability SaaS" 或 "task board" 的设计。Mutation 路径（answer blocker / resolve conflict / rewind）推迟到 v0.3 daemon API 之后，v0.2 Inspector 严格 read-only。
