...
| N-3 | 桌面 UI / 宠物形态 | v2 定位不锁死 UI 形态；v0.2+ 候选 |
...
这一节承认 v0.1 的 UX 形态尚未最终确定，但给出方向约束，避免在实施过程中走错方向。

**v1 的桌面宠物形态在 v2 不再是核心约束。** v2 的 Cairn 是内核，内核不需要有"脸"。

候选形态（v0.1 阶段以 CLI + MCP 为主，以下是 UI 层的方向建议，待 dogfood 后收敛）：

- **状态栏图标**：常驻状态栏（macOS Menu Bar / Windows 系统托盘）。正常状态图标安静，有冲突或需要用户决策时图标变色 + 系统通知。
- **Inspector 面板**：按需展开的只读面板，展示当前活跃 agent、最近 checkpoint、冲突历史。不是默认打开的仪表盘，是用户主动"看一眼"时用的。
- **系统通知**：冲突发生、越界预警、rewind 完成时的系统级通知（不劫持焦点，用户有空了再看）。
- **CLI / MCP 工具**：v0.1 的主交互层。8 个已落地的 MCP 工具 + Inspector NL 接口 + 进程总线查询。

v0.2 决策点：是否做 Inspector 面板的 GUI，或者保持 CLI-first 策略。决策依据：v0.1 dogfood 里用户主动查询 Cairn 的频率。如果查询频率高，做 GUI 有价值；如果用户大多数时候只需要通知，GUI 是过度投入。

具体界面形象**不在 v0.1 锁定**。v0.2 再定。
...
- Inspector 面板 UI（取决于 dogfood 反馈）
- 若有需求，桌面宠物轻量形态
