...
**关键设计约束（v0.1 重要边界！）**：
- v0.1 的回溯**只覆盖文件系统和 git 状态**。
- v0.1 **不覆盖**"Cairn 自己的记忆/对话状态"——即便回到 `t1`，对话里仍然留着它后来的发言。记忆快照（memory checkpoint）推迟到 v0.2。
- 这个限制必须在 UI 里对用户**明示**，不能让用户以为"点回溯就 100% 干净地回去了"。
...
总周期 **13 周 = 10 周桌面宠物主体 + 3 周 MCP scratchpad 楔**。

顺序是：**先 3 周 MCP 楔，再 10 周桌面主体**。理由：
- 楔形前奏产出一个能被**现有 Claude Code / Cursor 用户**立刻试用的东西，获得早期反馈。
- 验证"checkpoint + rewind"这一核心机制，在进入 10 周主体之前收口技术方案。
- 如果楔没人要，是一个便宜的 kill signal（见 §11）。
...
- W12：CLI；端到端三条用户故事（US-1, US-2, US-4）跑通。
- W13：Beta 发布、文档、ship。

### 9.2 v0.2（预计 +3 月 - +6 月）

- 记忆 checkpoint（解除 §4.4 的边界）。
- 宠物形象正式稿 + 表情/动作系统。
- 若干任务模板。
