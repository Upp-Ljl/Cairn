...
**支撑底座**（kernel 工程已交付）：29 MCP tools / 10 migrations / 8 host-level state objects / 439 daemon tests / 424 mcp-server tests / 32-of-32 cross-session dogfood PASS。
...
| **Integration layer** | MCP tools（现）/ future adapters / sidecars | agent 接入 Cairn 的方式 | ✅ MCP 29 工具已交付 |
...
任何 Cairn capability 都建立在这 8 类持久状态对象之上。它们是 Cairn 的"内核数据结构"，agent 通过 29 个 MCP 工具读写它们：
...
信号按 **6 个 `~~category` 占位符**组织（而不是写死的工具列表），mentor-collect.cjs 用 `CATEGORY_ALIASES` 把短别名映射到 canonical category name。每条 prompt 引用 `~~<category>` placeholder，由 collector 替换成该 category 下的真实片段。理由：**tool-agnostic**——日后新增 issue tracker（GitHub Issues / Linear / Jira）只是加一个新 category（如 `~~issue-tracker`），prompt 模板和 caller code 都不动；类似地砍掉某个 category 也不破坏调用方。category-placeholder 模型借自 `docs/superpowers/analyses/2026-05-15-cc-pm-plugin-vs-cairn.md` §4.1（HIGH borrowable pattern）。

| Category | 内容 |
|---|---|
| `~~project-narrative` | 项目静态叙述：whitelisted top-level docs（`PRODUCT.md` / `README.md` / `TODO.md` / `CLAUDE.md` / `ARCHITECTURE.md`） |
| `~~vcs-signal` | git HEAD / 当前 branch / `status --short` / 最近 20 行 `log --oneline` |
| `~~candidate-pipeline` | Three-Stage Loop 的 candidates JSONL（Mode B authorized work queue） |
| `~~iteration-history` | 历史 worker run iterations 摘要 |
| `~~worker-reports` | 结构化 agent self-report（finished / remaining / blockers / next-steps） |
| `~~kernel-state` | kernel SQLite 聚合 counts：`tasks_running / blocked / failed / waiting_review` + `outcomes_pending / failed` + `blockers_open` |

每个 category 用户可在 project `CAIRN.md` 里通过 `## Signals` section 单独 off（详 `docs/CAIRN-md-spec.md`）；默认全开。OAuth / 远端 source（GitHub Issues 等）暂不在 v4 首版 category 列表内，列入 Later。
...
（plan 该长啥样 / advisor 输出 shape 是啥 / handoff protocol 是啥）从
hardcoded prompt 字符串迁移到 `~/.cairn/skills/*.md` markdown 文件，
...
Defaults 在 `packages/desktop-shell/skills-defaults/`；首次 panel 启动
+ 每次 `cairn install` 自动 bootstrap 到 `~/.cairn/skills/`（idempotent，
已存在用户文件不覆盖）。Loader: `packages/desktop-shell/skills-loader.cjs`
（pure-fs，mtime-gate cache，graceful degrade）。per-project `CAIRN.md`
（`## Plan Shape` / `## Plan Hard Constraints` / `## Plan Authority`）
仍可 override / append 全局 skill。

这条迁移**不引入新 schema / 新 MCP tool / 新 host-level state**——纯
prompt engineering 工程改造，让"什么算 good output"这条知识从代码沉到
markdown，对齐 Cairn 的 "Customize by editing text not code" 哲学
（README）。

**边界保留**：`mentor-prompt.cjs::buildHardRules` 的 9 条 STRICT RULES
（permission boundary：no-real-names / no-files-not-listed / no-credentials
等）**留在代码里**，不外置——这是安全 bar 不是 quality bar，不接受
用户运行时编辑。

详 `docs/superpowers/analyses/2026-05-15-mentor-pattern-from-plugin.md`。
...
├── mcp-server/     # 29 MCP 工具，stdio (kernel + integration layer)
...
**累计**：29 MCP tools / 10 migrations / 8 host-level state objects / daemon 439 tests / mcp-server 424 tests / cross-session dogfood 32/32 PASS。
...
**D9.4 Mode A 「无底线 spawn」边界（CEO 鸭总 2026-05-14 拍板）**

Mode A 切开后 Cairn 在 managed project 目录 spawn `claude` 子进程，传 `--permission-mode bypassPermissions`，CC 在该项目可以**无 permission 弹窗执行任何动作**：写文件、跑命令、`git push`、`rm`、调外部 API 等。这是 Mode A 的产品契约：**用户切到 A 即明示「我无底线信任 CC 在此 project 内的判断」**。

边界：

- **作用域只在 managed project**。Cairn 自己的代码 + 注册表 + 内核状态仍受 CLAUDE.md 硬线保护（force push / amend / `--no-verify` / npm publish / 改 LICENSE 必须问 CEO）。
- **CAIRN.md authority 不是安全网**。当前 `mentor-project-profile.cjs::loadProfile` 用字面前缀（🛑 / `escalate:`）匹配，不做语义解析。「never force-push to main」这种 NL 表达不会被识别。用户切到 A 必须自知此 risk。
- **Cairn 自身不引导 CC 走破坏路径**。boot prompt（`mode-a-spawner.buildBootPrompt`）和 mentor auto-answer（`mode-a-auto-answer.cjs`）不会指示 CC 跑 destructive commands；命令由 CC 自主决策。Cairn 这一层的约束是「别瞎拼 prompt」。
- **Mode B 不受此约束**。Mode B 走「Mentor 排序建议 + 用户手点派单」路径，所有 dispatch 都经过用户确认；CC 启动时不带 `bypassPermissions`。
- **首次切 A 应有 UI 一次性提示**（per-project）。Later 工程：panel 在 mode toggle 第一次切到 A 时弹一句话「Mode A 授权 CC 在本项目无人值守执行——仅限你信任的项目」+ 一个 "明白了" 按钮 + 记 `cockpit_settings.auto_ship.mode_a_acknowledged_at`。

Sources: subagent verdict 2026-05-14 stream-json review #4。鸭总拍板「初期无底线信任 CC，Cairn 别瞎引导」，原话归档此处。
...
### 17.4 累计当前工具清单（29 工具，与 ARCHITECTURE.md §5 对齐）

scratchpad ×4 / checkpoint ×2 / rewind ×2 / process ×4 / conflict ×3 / inspector ×1 / dispatch ×2 / task ×8 / outcomes ×2 / session ×1 = **29 tools**。完整 alphabetical list 在 `tests/stdio-smoke.test.ts` 锁定。
