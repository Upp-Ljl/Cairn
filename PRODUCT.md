...
3. **Cairn 不是桌面宠物**（v0.1 不做，v0.2+ 候选）。宠物形态曾是 v1 的核心形象，v2 放弃这个定位。UX 候选形态是状态栏图标 + Inspector 面板，见 §8。
...
7. **Cairn 悬浮标不是桌面宠物**。v2 的悬浮标是 Inspector 通道的视觉载体（静物 / 路标隐喻 / 不接需求），不是 v1 那种拟人化 agent 化身。任何让悬浮标"接受用户开发任务""做表情动画""发声"的设计回到 v1，已废止。详 §8.2。
...
### 8.2 悬浮标（Floating Marker，v0.2 主形态）
...
**视觉**：
- 形象隐喻：路标 / 玛尼堆（cairn 本义，呼应名字）。静物，不做拟人化、不做表情、不发声
- 状态色编码：idle（中性灰）/ has-conflict（暖色脉动）/ busy（流光）/ DB-down（淡红警示）。色调微变化，不做夸张动画
- 不打扰：不夺焦、不弹窗、不发声
...
| 是否写代码 | 是 | **不**（v2 硬底线） |
| 视觉 | 拟人 / 拟物动画化 | 静物 / 抽象（克制） |
| 浮窗展开后占屏 | 400×600 长驻 | 480×600 按需 |

**技术形态**：Tauri（Rust + WebView）。理由：
- 体积小（~5-10MB），符合"安静常驻"产品调性
- 性能好（启动 < 200ms 是 Tauri 典型量级）
- 跨平台（macOS/Windows/Linux）
- 与 v0.1 daemon 库（Node + better-sqlite3）通过本地 IPC 或共享 SQLite 文件通信，daemon 不变

不选 Electron（80-150MB 体积过大）/ Native+WebView（每 OS 一套，工作量 ×3）。详 ARCHITECTURE.md ADR-8。

**v0.1 阶段**：不做悬浮标。v0.1 临时方案 = 状态栏图标 + 系统通知 + CLI（见 §8.1）。v0.2 启动悬浮标实施。

### 8.3 反例（什么样是做错了）

以下设计违背 v2 硬底线，出现时直接用 §1.3 第 7 条 veto：

- 悬浮标默认就是表情动画化的小动物 → 错（回到 v1）
...
8. **W3 dogfood 的发布节奏**：原计划 W3 做私有 release，5-10 个早期用户。当前用户倾向缓发，等更多功能落地。确切时间待定，不强行 W3。
...
| Tool | 语义 | 对应能力 | 阶段 |
|---|---|---|---|
| `cairn.inspector.query(nl_query)` | 自然语言查询当前状态（Inspector 通道） | 冲突可见 + Inspector | W3-W5 |
| `cairn.conflict.list([since])` | 列出冲突历史 | 冲突可见 | W3-W5 |
| `cairn.process.register(agent_id, ...)` | agent 注册进程总线 | 进程总线 | W4-W6 |
| `cairn.process.heartbeat(agent_id)` | agent 心跳上报 | 进程总线 | W4-W6 |
| `cairn.process.list()` | 查询当前活跃 agent 列表 | 进程总线 + Inspector | W4-W6 |
| `cairn.dispatch.request(nl_intent)` | 提交 Dispatch 请求（NL→意图解析→待确认） | 需求可派 | W5-W7 |
| `cairn.dispatch.confirm(request_id)` | 确认 Dispatch 并转发给 agent | 需求可派 | W5-W7 |
...
- W3：打包发布、写 README、向 5-10 个早期用户做 private release。
