...
详见 `docs/w2-exit-review.md` §1 条目 6/7/9。

**对 W4 决策的影响**：
- 三条路径中，**路径 C（outward agent + lanes 监控）依赖装机率最大** — 要观察用户的 lane state 行为
- 路径 A 和 B 在 0 装机情况下也能推进（自用 + dogfood 即可验证）
- W3 期间通过 T3.6a 拿到 1-2 个针对性私信反馈即可，不要求规模
