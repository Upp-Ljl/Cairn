...
| 15 | `subagent_running_misread_as_stop` | Treated "a sonnet subagent is running in the background" as a stop condition for the main agent. **It is not.** Subagents in background only own the files they declared in their prompt; the main agent has the rest of the codebase to work on. When a subagent is in flight: identify the next biggest item in the authorized scope whose file set does NOT overlap the subagent's, and start it. Per CEO correction 2026-05-14 ("为什么又停了" 第九次踩). Examples of legitimate parallel work while a subagent runs: new feature in a different package, doc updates, plan writing, real-LLM dogfood of just-shipped features, agent self-proposals via live MCP, structural smokes for new modules, release notes draft. **NEVER** end a turn with "subagent 跑着，等通知" alone — that surfaces the same dressed-up "asking permission" failure as field 13/14. Action when a subagent goes async: (a) identify N+1 parallel item, (b) start it, (c) when subagent notification arrives, merge it AND continue the parallel work without surfacing a "subagent done" status report. |
