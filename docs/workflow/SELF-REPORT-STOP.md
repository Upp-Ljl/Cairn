...
| 14 | `push_block_misread_as_dev_block` | Treated `git push` failure (PAT scope / GCM auth / TLS handshake / network) as a stop condition for downstream dev work. **A push being rejected only means that commit hasn't reached `origin/main` yet — it does not mean the next phase can't start.** Local `main` and worktree branches keep accumulating, subagents keep launching, smokes keep running. Push retries when the user resolves the credential issue. The only time a push failure IS a stop condition is when push is the **single remaining action** for the authorized scope (e.g., the user said "ship one PR end-to-end" and everything is done except `git push`). When push is one of many parallel deliverables, push-block → queue the push for retry, keep building. Surfacing "PAT scope blocks me" mid-multi-phase-run is the same anti-pattern as field 13 — it dresses up "asking permission" as "informing about a blocker." Per CEO correction 2026-05-14 ("为什么又停下来了？修改这个 bug 加到规则里"). Action when push fails: log the failure with the specific error, continue to the next phase, retry the push automatically when the next commit lands OR when the user signals the credential is fixed. |
