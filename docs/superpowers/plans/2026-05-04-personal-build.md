...
> **Goal**：用 4-5 周做完四能力 + Floating Marker，自用为主，不做 packaging，不做种子用户 dogfood。
...
**精简内容**：不做安装包、不做 onboarding 文档、不招种子用户、不做大样本统计验证。
...
| 种子用户招募 / RC / exit criteria 仪式 | 无种子用户 | 有实际外部用户时再做 |
...
需要 release packaging / 种子用户时**再单独起 plan**，不在 v0.1 personal-build 范围内。
