...
```
v0.1 W11-W12 release packaging 需要解决的问题：
...
| v02-mvp-w3-**b** | 优先级排序 | 对 -a 整理的工作项，按"对 v0.1 完成路径的阻塞程度"排序（越阻塞越优先）；对每项估工作量（小：1-2 周 / 中：3-4 周 / 大：5-6 周）；特别标注 PoC-4 若调用率 < 70% 时的 path-b-wrapper（这会从"中"变"高优先"）；写入 scratchpad（含优先级列表 + 估算 + 前提条件依赖） |
