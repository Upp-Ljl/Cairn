...
│   │   ├── spawn-utils.ts                           # NEW — 唯一可 import `child_process` 的文件；`runWithTimeout` helper（§7.1 锁）
│   │   ├── path-utils.ts                            # NEW — realpath-based cwd containment，Windows 大小写归一化（§7.1 锁）
...
└── tests/
    ...
    │   ├── spawn-utils.test.ts                      # NEW — spawn / timeout / kill / output cap / ANSI strip
    │   └── path-utils.test.ts                       # NEW — realpath / traversal / Windows 大小写
...
#### Task 5.3.2：spawn-utils + path-utils（**Day 3 必须先于 primitives 完成的 prerequisite**）

**目标**：把所有 `child_process` 与 path-safety 收敛到 2 个工具模块。primitives 实现只关心语义（"这个原语在做什么"），不关心 OS 细节（spawn / kill / quoting / case-fold）。Day 3 fresh sonnet 起手第一件事是这个，**不**先写 primitives——绕开 spawn-utils 实现 7 个原语会重复发明半吊子 spawn/kill 逻辑。

**Step 0：spawn experiment（Day 3 必跑，验证 §7.1 v1 策略在你的 Windows + WSL/git-bash 上工作）**：
- 临时 fixture：`temp/spawn-experiment.mjs`（gitignored）按 §7.1 验证表跑 6 个 fixture：quick PASS / quick FAIL / timeout（≥ 2s 超时实际 kill 整树）/ quoted args / real `tests_pass` PASS / real `tests_pass` FAIL
- Windows + POSIX（WSL 或 git-bash）都跑过，确认 PID 真消失（`tasklist /FI "PID eq <pid>"` 或 `ps -p <pid>` 验证）
- 任一 fixture 失败 → 停下报告，**不**进 Step 1；当下决定是否引 `tree-kill` dep（plan §7.1 已留 escape hatch）

**Step 1：spawn-utils.ts**——按 §7.1 锁定的策略实现：
```ts
export interface SpawnResult {
  status: 'PASS' | 'FAIL' | 'TIMEOUT';
  exitCode: number | null;
  stdout: string;          // already capped to 64KB + ANSI-stripped
  stderr: string;          // same
  truncated: boolean;      // true if either stream was truncated
  elapsed_ms: number;
}

export async function runWithTimeout(cmd: string, opts: {
  cwd: string;
  timeoutMs: number;
  env?: NodeJS.ProcessEnv;
}): Promise<SpawnResult>
```
- `child_process.spawn(cmd, [], { shell: true, cwd, env, stdio: ['ignore','pipe','pipe'], windowsHide: true, detached: process.platform !== 'win32' })`
- Timeout：发起后启 `setTimeout(timeoutMs)`；超时调内部 `killChildTree(child, 5000)`：Windows 路径用 `taskkill /F /T /PID <pid>` (via `child_process.exec`)，POSIX 路径 `process.kill(-child.pid, 'SIGTERM')` → 5s 等 → `SIGKILL`
- Output capping：stdout/stderr 各 64KB；超量截断 + 在 truncated 字段标记
- ANSI strip：用纯正则 `/\x1b\[[0-9;]*m/g` 替换为空（不引 dep）

**Step 2：path-utils.ts**——cwd containment 实现：
```ts
export function assertWithinCwd(target: string, cwd: string): {
  ok: true;
  resolved: string;        // realpath of target (or target itself if not yet exists)
} | {
  ok: false;
  reason: 'TRAVERSAL' | 'OUTSIDE_CWD' | 'INVALID_PATH';
}
```
- 先 `fs.realpathSync(cwd)` 拿 cwd 真路径
- target 已存在 → `fs.realpathSync(target)`；不存在 → `path.dirname(target)` realpath + 拼 basename
- Windows 平台 `process.platform === 'win32'`：归一化两边到 lowercase 后再比较
- containment：`resolvedTarget === resolvedCwd || resolvedTarget.startsWith(resolvedCwd + path.sep)`

**Step 3：tests**——`tests/dsl/spawn-utils.test.ts` ≥ 10 case + `tests/dsl/path-utils.test.ts` ≥ 8 case：
- spawn-utils：6 spawn fixture 上面 Step 0 的镜像（在 vitest 里跑）+ ANSI strip + truncation + env passthrough
- path-utils：traversal `../../etc/passwd` 拒绝 / symlink 在 realpath 后越界拒绝 / Windows 大小写不同视为同一路径 / target 不存在但 parent 在 cwd → ok / target 完全不存在的孤儿路径 → INVALID_PATH

**Step 4：实现约束（grep 验证）**——所有 child_process 必须**只**经 spawn-utils：
```bash
grep -rn "child_process\|require.*child_process\|from .child_process." packages/mcp-server/src/dsl
# 期望：只在 spawn-utils.ts 内出现；primitives.ts / evaluator.ts / parser.ts 等绝不出现
```

- [ ] **DoD**：spawn experiment 6 fixture 在 Windows + POSIX 都 PASS；spawn-utils ≥ 10 case + path-utils ≥ 8 case 全绿；`grep "child_process" packages/mcp-server/src/dsl` 仅 spawn-utils.ts 命中（命名 import 也算）；超时路径下 PID 不残留

#### Task 5.3.3：7 个 primitive evaluator
...
- [ ] **Step 1**：交付 `primitives.ts` —— 每原语一个 async function。统一接口：
  ...
  **实现要点**（**所有子进程通过 `spawn-utils.runWithTimeout`，所有路径校验通过 `path-utils.assertWithinCwd`；不直接 import `child_process`**）：
  - **`tests_pass`**（P2.2 锁单一语义）：用 `assertWithinCwd(args.target ?? '.', ctx.cwd)` 校验；OK 后从该 resolved 目录读 `package.json`，取 `scripts.test`；调 `runWithTimeout(testCmd, { cwd: resolved, timeoutMs })`；exit 0 = PASS，非 0 = FAIL，TIMEOUT 透传。target 越界或 package.json/scripts.test 缺失 → FAIL with detail。**禁**接受任意 shell 命令——那是 `command_exits_0` 的工作
  - **`command_exits_0`**：直接 `runWithTimeout(args.cmd, { cwd: args.cwd ?? ctx.cwd, timeoutMs })`；spawn-utils 自带 shell:true，跨平台 quoting 自动；exit 0 = PASS。如 args.cwd 给定，先 `assertWithinCwd` 校验
  - **`file_exists`**：`assertWithinCwd(args.path, ctx.cwd)` → OK 后 `fs.existsSync(resolved)`；越界返 FAIL with reason
  - **`regex_matches`**：`assertWithinCwd(args.file, ctx.cwd)` → OK 后读文件（v1 max 16MB；超量返 FAIL "file too large"）+ 构造 RegExp（args.flags 默认 ''）+ test。文件不存在 / 正则编译失败 → FAIL with detail
  - **`scratchpad_key_exists`**：`SELECT 1 FROM scratchpad WHERE key = ? [AND task_id = ?]`（task_id 来自 args 或 ctx.task_id）；纯 DB 操作不经 spawn-utils
  - **`no_open_conflicts`**：`SELECT COUNT(*) FROM conflicts WHERE status = 'OPEN' [AND ... scope filter]`；scope_paths 用 LIKE 匹配 `conflict.paths_json`；纯 DB
  - **`checkpoint_created_after`**：`SELECT COUNT(*) FROM checkpoints WHERE task_id = ? AND created_at > ? AND snapshot_status = 'READY'`；纯 DB
  - **不**在 primitives.ts 内 spawn / kill / 写 timeout 逻辑——spawn-utils.ts 是唯一 owner（Task 5.3.2 锁）。timeout / kill / output cap / ANSI strip / Windows taskkill 全部在那个模块内置
...
- [ ] **DoD**：≥ 21 case 全绿；子进程类原语在超时情况下不留 zombie 进程（process list 验证；实际由 spawn-utils 保证）；**`grep "child_process" packages/mcp-server/src/dsl/primitives.ts` 必须为空**——所有 spawn 经 spawn-utils

#### Task 5.3.4：DSL evaluator（聚合层）
...
- 子进程类原语在超时时**必须**杀进程链：`SIGTERM → 5s grace → SIGKILL`（实际由 §7.1 spawn-utils 内置）
- 子进程 stdout/stderr 限制最多 64KB（避免 OOM 写入 evaluation_summary；由 §7.1 spawn-utils 实施）

---

## 7.1 Cross-platform spawn strategy（实现约束 — Day 3 锁）

子进程执行 + path containment 是 Phase 3 最容易出跨平台细节问题的一块。本节是 Day 3 实现的**强约束**，由 Task 5.3.2 (`spawn-utils.ts` + `path-utils.ts`) 兑现。primitives.ts 不允许直接调 `child_process` / 自己写 timeout-kill / 自己做 path 校验——所有这些都收敛到这两个工具模块。

### 7.1.1 实现约束（grep 强制）

```bash
# Day 5 / Task 5.6.4 Step 2 必跑：
grep -rn "child_process\|require.*child_process\|from .child_process." packages/mcp-server/src/dsl
# 期望：只在 spawn-utils.ts 文件内出现；primitives.ts / evaluator.ts / parser.ts 等绝不出现

grep -rn "fs.realpath\|path.resolve.*startsWith" packages/mcp-server/src/dsl/primitives.ts
# 期望：空。所有 path 校验只能调 path-utils.assertWithinCwd
```

任何 primitive 实现内联 spawn / kill / containment 检查 = 违反 plan，code review 必拒。

### 7.1.2 Spawn 调用形态

```ts
// packages/mcp-server/src/dsl/spawn-utils.ts
import { spawn, exec } from 'node:child_process';   // 整个 dsl/ 目录里**唯一**这一个 import
...
export async function runWithTimeout(cmd: string, opts: {
  ...
  const child = spawn(cmd, [], {
    shell: true,                                       // 自动选 cmd.exe (Win) / /bin/sh (POSIX)；scripts.test 是 shell string 必须经 shell
    cwd: opts.cwd,
    env: { ...process.env, CI: '1', NO_COLOR: '1', FORCE_COLOR: '0', ...opts.env },
    stdio: ['ignore', 'pipe', 'pipe'],                 // stdin closed → 防交互式 hang
    windowsHide: true,                                 // 不弹 Windows console window
    detached: process.platform !== 'win32',            // POSIX 起独立进程组（kill -PID 杀整组）；Windows 走 taskkill 路径
  });
  // ... timeout setTimer + output capping + ANSI strip + killChildTree on timeout
}
```

**Why `shell: true`**：`tests_pass` 从 `package.json scripts.test` 读到的是字符串（"vitest run" / "npm run test:unit"），直接 spawn-without-shell 跑不通；`shell: true` 接管 quoting + arg 分割。代价：命令字符串 shell-aware（用户 / 项目自己保证），DSL 不试图 magic-translate。

### 7.1.3 进程终止 — 平台分支（v1 不引依赖）

**核心问题**：Windows 上 `child.kill()` 只杀直接 child；嵌套子进程（npm test → vitest → workers）会成为孤儿。POSIX 用进程组方案天然解决；Windows 必须用 `taskkill /F /T`。

```ts
async function killChildTree(child: ChildProcess, graceMs = 5000): Promise<void> {
  if (process.platform === 'win32') {
    // taskkill /F /T 杀整个 PID 树 —— Windows 内置，不需 dep
    await new Promise<void>((resolve) => {
      exec(`taskkill /F /T /PID ${child.pid}`, () => resolve());
    });
  } else {
    // POSIX: spawn 时 detached:true 起了进程组，kill 负 PID 杀整组
...
**已知边界**：Windows 上 `taskkill /F /T` 对正常 PID 树有效；如果 child 在被 kill 前 detach 出某个 background service（罕见，npm test 不会），那个服务可能不被收。Phase 3 v1 接受这个边界。如果 dogfood / 真实使用观察到 zombie，Phase 4 / v0.2 引 `tree-kill` dep（escape hatch 已在风险表）。

### 7.1.4 Output 捕获

- stdout / stderr 各 64KB 上限。超量截断 + 在 `SpawnResult.truncated = true` 标记，evaluation_summary 末尾自动追加 "[output truncated, full discarded]"
- ANSI escape strip（防御性，即使有 `NO_COLOR=1` + `FORCE_COLOR=0`）：纯正则 `text.replace(/\x1b\[[0-9;]*m/g, '')`，不引 `strip-ansi` dep

### 7.1.5 Path containment — `path-utils.ts`

朴素的 `path.resolve(target).startsWith(cwd)` **不够**——symlink 可以绕，Windows 大小写不敏感会漏。v1 至少：
...
export function assertWithinCwd(target: string, cwd: string): {
  ...
  const realCwd = caseFold(fs.realpathSync(cwd));            // realpath 解析所有 symlink
  let realTarget: string;
  try {
    if (fs.existsSync(target)) {
      realTarget = caseFold(fs.realpathSync(target));        // 已存在 → 全路径 realpath
    } else {
      // 不存在 → parent realpath + basename，避免 unresolvable path 直接 throw
      const parent = path.dirname(target);
      realTarget = caseFold(path.join(fs.realpathSync(parent), path.basename(target)));
    }
  } catch {
...
**Windows 大小写归一化**至关重要：`C:\Users\foo` 和 `c:\users\foo` 是同一文件系统路径，但 `startsWith` 会判 false 否定（或相反）。

### 7.1.6 Day 3 Step 0 spawn experiment（先验证再实现）

Day 3 fresh sonnet 起手 **第一件事**是写 `temp/spawn-experiment.mjs`（gitignored），按下面 6 fixture 跑：

| Fixture | 命令 | 期望 |
|---|---|---|
| Quick PASS | `node -e "process.exit(0)"` | PASS in <1s, exitCode=0 |
| Quick FAIL | `node -e "process.exit(1)"` | FAIL, exitCode=1 |
| Timeout（kill 整树）| `node -e "setInterval(()=>{}, 1000)"` + 2s timeout | TIMEOUT verdict；测试结束后 `tasklist /FI "PID eq <pid>"` (Win) 或 `ps -p <pid>` (POSIX) 验证 PID 不存在 |
| Quoted args with spaces | `node -e "console.log('hello world')"` | PASS, stdout 含 "hello world" |
| Real `tests_pass` PASS | 在 packages/daemon 跑 `npm test` 子集 | PASS |
| Real `tests_pass` FAIL | 临时改个测试为 red 后跑 npm test | FAIL with stderr 含 red test 名字 |

**Windows + POSIX（git-bash 或 WSL）都跑**。

任何 fixture fail（特别是 Timeout 后 PID 残留）→ **停下报告**，不进 Task 5.3.2 Step 1；当下决定是否引 `tree-kill` 或调整策略。

### 7.1.7 dependency footprint

Phase 3 v1 spawn-utils + path-utils 实现**不引任何新 npm 依赖**。仅用 Node 内置：
- `node:child_process`（spawn / exec）
- `node:fs`（realpathSync / existsSync）
- `node:path`（resolve / dirname / basename / sep）

如果未来真因为 Windows 边界问题需要引 `tree-kill`，那是 Phase 4 / v0.2 的事，当下不做。
...
| 风险 | 概率 | 影响 | 缓解 |
|---|---|---|---|
| 子进程超时未真正 kill，留 zombie | 中 | 高 | LD-16 + §7.1：Windows 用 `taskkill /F /T /PID`，POSIX 用 detached process group + 负 PID kill；spawn-utils.ts 是唯一 owner；Day 3 Step 0 spawn experiment 在 Windows + POSIX 双平台验证 PID 真消失 |
| **Windows shell quoting / taskkill 边界**（child detach 后子服务不被收 / cmd.exe 引号差异 / 命令含 `&&` 在 PowerShell 5.1 解析失败） | 中 | 中 | §7.1 锁定 `shell: true` 让 Node 选 cmd.exe（非 PowerShell）兜底引号；`tests_pass` 不组装命令只透传 `scripts.test`；`command_exits_0` 是 power tool 不替用户 escape；如真观察到 detach 出 background service 不被 taskkill 收，§7.1.7 留 escape hatch 引 `tree-kill` dep |
...
| **primitives.ts 临时发明 spawn / kill / containment 逻辑**（绕过 spawn-utils） | 中 | 高 | §7.1.1 锁：`grep "child_process" packages/mcp-server/src/dsl/primitives.ts` 必为空；Day 3 Task 5.3.2 必须先于 5.3.3 完成（spawn-utils 先存在 primitives 才能 import）；code review 强制查 |
| Path traversal in `file_exists` / `regex_matches` / `tests_pass.target` | 中 | 中 | §7.1.5 `assertWithinCwd` realpath + Windows 大小写归一化；naive `startsWith` 不够（symlink 可绕、Windows 大小写不敏感会漏）；越界返 FAIL with reason='OUTSIDE_CWD' / 'TRAVERSAL' |
...
- [ ] `dsl/spawn-utils.ts` + 单测 ≥ 10 case 全绿（含 §7.1.6 spawn experiment 6 fixture 的 vitest 镜像 + ANSI strip + truncation + env passthrough）
- [ ] `dsl/path-utils.ts` + 单测 ≥ 8 case 全绿（含 traversal / symlink / Windows 大小写 / 不存在 target 的 parent realpath fallback）
- [ ] `dsl/parser.ts` + 单测 ≥ 12 case 全绿（白名单严格、未知/缺字段/多余字段拒绝）
- [ ] `dsl/primitives.ts` + 单测 ≥ 21 case 全绿（每原语 happy/sad/edge ≥ 3）
- [ ] **§7.1.1 grep 强制约束**：`grep -rn "child_process" packages/mcp-server/src/dsl` 仅 `spawn-utils.ts` 命中；`grep -rn "fs.realpath\|path.resolve.*startsWith" packages/mcp-server/src/dsl/primitives.ts` 必为空（所有 path 校验经 `path-utils.assertWithinCwd`）
- [ ] **§7.1.6 Day 3 Step 0 spawn experiment** 在 Windows + POSIX 双平台 6 fixture 全 PASS；timeout 路径 PID 不残留（`tasklist` / `ps` 验证）
