Create a single horizontal sprite strip for the Codex app digital pet `cairn` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: A six-stone cairn trail-marker stacked 3+2+1, pixel art Stardew Valley sprite quality, transparent-background-ready. Strict cool palette only: slate blue #2C3E50, dark stone grey #374151, cool grey #4B5563, blue-grey #64748B, cool slate #475569, ice grey-white #CBD5E1, frost white #F1F5F9. No warm colors, no browns. All six stones have different irregular polygonal silhouettes, slight random rotations, hand-stacked feel. Bottom layer 3 stones foundation, middle 2 stones transition, top 1 stone smallest and brightest with two black eyes, no mouth. The top stone is the protagonist; bottom 5 stones are silent foundation..
