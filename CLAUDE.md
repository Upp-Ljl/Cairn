...
- **改 UI / 视觉 / CSS / 动效前必读 `DESIGN.md`**。所有颜色 / 字体 / 间距 / 动效 token、组件样式、状态色语义都在那里。色彩规则核心：只用 accent（蓝）+ alert（红）+ 灰阶，**禁止**绿/琥/紫作为独立状态色。改 DESIGN.md 必须先问用户。预览页 `packages/desktop-shell/dev/design-preview.html`（dev server 启动后访问）。
...
- **Mode A stream-json launcher**（Phase 1+2+3，2026-05-14；hooks turn protocol，2026-05-15）：Mode A 用 `claude --output-format stream-json --input-format stream-json --verbose --permission-mode bypassPermissions --mcp-config <tmp> --strict-mcp-config --settings <tmp-hooks> --include-hook-events [--resume <id>]` 替换老的 `--print`。
  - `packages/desktop-shell/claude-stream-launcher.cjs` — 双向 stdio；stdin 一直开；stdout NDJSON 实时解析 → `~/.cairn/worker-runs/<run_id>/stream_events.jsonl`（raw）+ `tail.log`（仅 assistant text）+ `hook-events.jsonl`（audit trail）；10 分钟 idle watchdog；session_id 从 Stop hook payload 抽出（primary），result event 兜底。
  ...
  - Smoke：`scripts/smoke-stream-launcher.mjs`（62 断言）/ `scripts/smoke-mode-a-session-store.mjs`（39）/ `scripts/smoke-mode-a-spawn-resume.mjs`（28，integration with fake claude binary）/ `scripts/smoke-claude-settings-config.mjs`（15，hooks settings builder）/ `scripts/smoke-hooks-turn-protocol.mjs`（41，hooks E2E + malformed payload + double-stop dedupe）。
- **Hooks turn protocol**（2026-05-15）：`packages/desktop-shell/claude-settings-config.cjs` 写 per-spawn 临时 `settings.json` 注入 SessionStart + Stop hooks（参考 smithersai/claude-p）。hook command = `node -e "..."` 把 CC stdin 的 payload **同时**写 `~/.cairn/worker-runs/<runId>/hook-events.jsonl` (durable audit) + echo 回 stdout (CC 把它放进 `hook_response.stdout` NDJSON event)。launcher 解析后 fire `onTurnDone({source, turn_index, session_id, last_assistant_text, transcript_path, stop_hook_active, raw})`。dedupe：每个 `turn_index` 只 fire 一次——`stop_hook_active=true` 的 Stop（reentry，CC 继续干活）跳过；后续 `stop_hook_active=false` 的 Stop 触发一次；同一 turn 的重复 `active=false` 也被 `stop_hook_dup_suppressed` log 截断。Architecture B 接 multi-turn 时 `writeNextTurn` 要 bump `turn_index` 才能让下一 turn 的 Stop 再 fire（writeNextTurn JSDoc 有 TODO）。fallback：CC 不发 hook event 时退到 result-event capture（source='result'）。spike `D:/lll/cairn/scripts/spike-claude-hooks.mjs` 9.9s Windows-verified。
...
- **Mentor signal categories — `~~category` placeholder 词汇**（2026-05-15，commit `57c5502` on main）：Mode A Mentor 的 prompt 信号按 6 个 tool-agnostic category 占位符组织，不是写死的工具名。canonical 名 = `~~project-narrative / ~~vcs-signal / ~~candidate-pipeline / ~~iteration-history / ~~worker-reports / ~~kernel-state`；短别名 → canonical 映射在 `packages/desktop-shell/mentor-collect.cjs::CATEGORY_ALIASES`。用户可在项目 `CAIRN.md` 的 `## Signals` section 单独 off 某 category（详 `docs/CAIRN-md-spec.md`），默认全开。新增 source（GitHub Issues / Linear 等）= 加 category，prompt 模板和 caller code 不动。
- **Skill 外置（2026-05-15）**：Scout / Mentor advisor / Lead-CC boot 三处的"质量 bar"硬编 prompt 字符串外置到 `~/.cairn/skills/{plan-shape,mentor-recommendation,handoff-protocol}.md`。Loader = `packages/desktop-shell/skills-loader.cjs`（pure fs，mtime-gate cache，graceful fallback 到 5-line embedded default 如果文件丢失）。Defaults 在 `packages/desktop-shell/skills-defaults/`；首次 panel 启动 + 每次 `cairn install` 自动 bootstrap，不覆盖已有用户文件。**`mentor-prompt.cjs::buildHardRules` 的 9 条 STRICT RULES 保留代码里**——这是 security boundary 不外置。模式参考 smithersai 类 plugin 的 skill 心法 + 分析文档 `docs/superpowers/analyses/2026-05-15-mentor-pattern-from-plugin.md`。智能用户改 skill md 调全局 default；CAIRN.md `## Plan Shape` 等仍可 per-project override。Smoke: `smoke-skills-loader.mjs` (24)。
