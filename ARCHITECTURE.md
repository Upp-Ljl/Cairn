...
> 版本：v0.1（kernel layer 全部交付 + product layer Quick Slice MVP 已落地）
> 最近更新：2026-05-08
> 状态：4 能力 + Task Capsule + Outcomes DSL 全部实施完毕（kernel）；desktop-shell 升级为 panel + pet + tray（product layer Quick Slice）；本文为 v0.1 的实现层权威文档
...
- **PRODUCT.md v3**（2026-05-08 reframe，commit `3562f1f`）：产品定义，回答"做什么、为什么这样定位、能力 + 反定义是什么"。v3 锁定双层 framing：product layer = project-scoped agent work side panel / project control surface；kernel layer = host-level multi-agent coordination kernel。本文不重复产品理由，只在必要时引用节号。
...
Cairn 是双层产品（PRODUCT.md v3 §1.2 四层架构 = Product / Kernel / Integration / Storage 4 layers always-on simultaneously）：

- **agent 写路径（kernel + integration + storage）**：Claude Code / Cursor / Aider / Cline / Claude Code Task subagents → MCP stdio → mcp-server → daemon storage → SQLite。daemon 是唯一写者，SQLite 是状态权威。Cairn 不向上代理 agent 推理，不调度 lead-subagent，不替 agent 执行任务。
- **用户读路径（product layer）**：desktop-shell → SQLite read-only connection → panel.html / pet / tray icon。**desktop-shell 不通过 MCP**——MCP 是 agent ↔ daemon 通道，不是 desktop ↔ daemon。WAL 模式让 panel 的 reader 不阻塞 mcp-server 的 writer。
...
```
  ...
  │                                      ▼                         │
  │                             ┌─────────────────────────────────┐│
  │                             │   Cairn desktop-shell (Electron)││
  │                             │   packages/desktop-shell/        ││
  │                             │                                  ││
  │                             │   panel.html  · summary card +  ││
  │                             │                Run Log + Tasks  ││
  │                             │   tray icon   · idle / warn /   ││
  │                             │                alert (1s poll)  ││
  │                             │   pet         · ambient sprite  ││
  │                             │   legacy      · inspector-legacy││
  │                             │                                  ││
  │                             │   strict read-only by default;  ││
  │                             │   CAIRN_DESKTOP_ENABLE_MUTATIONS││
  │                             │   gates the legacy Resolve path ││
  │                             └─────────────────────────────────┘│
  └─────────────────────────────────────────────────────────────────┘
```
...
- **product layer 严格 read-only**（PRODUCT.md v3 §12 D9）：desktop-shell 默认不暴露任何 mutation channel；panel.html 永远不渲染 mutation 按钮；legacy Inspector 的 Resolve 按钮仅在 `CAIRN_DESKTOP_ENABLE_MUTATIONS=1` 时渲染（兼容 `dogfood-live-pet-demo.mjs`）。`grep '\.run\|\.exec' packages/desktop-shell/*.cjs *.js` 必须 ≤ 1 处。"从 panel 答 blocker / rewind / dispatch" 的解锁有三个前置条件——daemon 独立进程 API、supervisor identity 模型、dogfood 信号——没全亮就不做。
- **不是 Cursor clone / Jira clone / IDE / plain MCP service**：见 PRODUCT.md §1.3 完整 9 条反定义。Product layer 的形态参照系是 Activity Monitor / Task Manager / `journalctl --follow`，**不**参照 Cursor / VS Code / Jira / Linear / Asana / Slack。

### 1.4 Pipeline autonomy contract / 无人值守契约

> Canonical doc: [`AUTOMATION.md`](AUTOMATION.md) + [`docs/unattended-pipeline.md`](docs/unattended-pipeline.md).

**Cairn 的 pipeline 是 unattended by design**——`grilled idea → cairn.task.create → DONE`（或 terminal `FAILED`）整条路径**不依赖人类在中间介入**。§1.3 里出现的"用户 confirm"、"用户拍板"等词是**默认 UX**，不是**契约**：

- **State machine 不是 human gate**：`VALID_TRANSITIONS`（`packages/daemon/src/storage/tasks-state.ts`）是一份"哪种 state → 哪种 state 合法"的纯数据契约，**不区分调用方身份**。任何能调 MCP 工具的进程（另一个 agent / scheduled retry / fallback policy / resolver thread / 人类）都可以驱动转移。
- **`dispatch.confirm` / `task.answer` / `outcomes.evaluate` / `conflict.resolve` 是 caller-agnostic tool calls**：当文档说"用户 confirm"时，意思是"caller confirms"——human 是 caller 的一种但不是唯一一种。agent-to-agent auto-confirm 是支持的（对非 irreversible intent），R1 fallback rule 强制 destructive intent 经过 `rewind.preview`（一个确定性 dry-run，不是人类点击）。
- **三个 apparent HITL state 的 escalation path**（不是 blocking gate）：
  - `BLOCKED` task → `cairn.task.answer` 任何 caller 都能调；`resume_packet` 给冷启动 agent 提供完整 context（open + answered blockers + scratchpad keys + frozen criteria + audit）
  - `WAITING_REVIEW` outcome → `cairn.outcomes.evaluate` 同步跑 7 个 deterministic DSL primitives（AND 聚合，**no LLM judge**），PASS → DONE / FAIL → RUNNING 重跑（criteria 冻结）
  - `PENDING_REVIEW` conflict → recorded but **non-blocking**；resolver agent async 读 `cairn.conflict.list` 调 `cairn.conflict.resolve`
- **"用户/agent" 在 §1.3 / §4.x 是同一个语义槽**：当 README / PRODUCT.md / ARCHITECTURE.md 出现"用户 confirm" / "用户答 blocker" / "用户裁决冲突"等措辞时，应等价读作"caller confirms / caller answers / caller arbitrates"。Desktop side panel 是 read-only 观察面（D9 lock），它是这条 pipeline 的**观察窗**，不是**步骤**。

终端 outcome 只有：`DONE`（验收通过）/ `FAILED`（`outcomes.terminal_fail` 显式放弃 或 `outcomes.evaluate` 终判失败 或 `RUNNING → FAILED`）/ `CANCELLED`（`cairn.task.cancel`）。**不存在"无限等待人类"状态**。
...
### 3.2 已落地（kernel layer + product layer 全部）
...
│   │   └── index.ts           # 占位（独立 daemon 进程入口是 Later 项）
...
│
└── desktop-shell/             # Product layer：Electron panel + pet + tray (Quick Slice MVP)
    ├── main.cjs               # Electron lifecycle + window mgmt + IPC + Tray + 1s polling
    ├── preload.cjs            # contextBridge; mutation channel exposed only under env flag
    ├── queries.cjs            # read-only SQL helpers + 5 JSDoc typedefs
    ├── panel.html / panel.js  # Quick Slice main UI: workspace label / summary card /
    │                          # Run Log (default tab) / Tasks tab w/ inline expansion
    ├── inspector-legacy.{html,js}  # legacy 4-section view; Resolve hidden by default
    ├── preview.html / preview.js   # pet sprite renderer (schema → animation contract)
    ├── state-server.js        # browser-mode HTTP fallback (debug)
    ├── scripts/
    │   └── mvp-quick-slice-dogfood.mjs   # --setup / --status / --cleanup, idempotent
    ├── SCHEMA_NOTES.md        # live DB schema reference for the 6 tables panel reads
    └── README.md
```

**跨包 import 规则**：

1. `mcp-server` import `daemon` 走 `../../daemon/dist/`，不走源码。已知技术债（见 §9），等 monorepo 工具引入时统一。
2. `desktop-shell` **不**通过 MCP 与 daemon 通信。它直接 require `better-sqlite3` 打开 `~/.cairn/cairn.db` 的 read-only 句柄；`queries.cjs` 是唯一 SQL 出口；`main.cjs` 仅做 Electron / IPC / window / tray。
3. `desktop-shell` 不引前端框架 / TS / bundler——Electron 32 + 原生 HTML/CSS/JS + better-sqlite3 是 frozen stack（subagent verdict 2026-05-08：渲染规模在框架盈亏点之下；信息密度优先的审美是 anti-framework）。新增 npm dep 需要重新评估。
4. `desktop-shell` 也读 `lanes` 表给 pet sprite 用（legacy schema），是已知 drift（PRODUCT.md §8.5 注 + Hardening 修）。新 panel 视图（panel.html）只读 8 host-level state objects 的 6 个目标表。

### 3.3 Later 候选新包（不绑版本号；PRODUCT.md §12 D10）
...
| `packages/cairn-daemon-svc/` | desktop-shell mutation 解锁需要 daemon 独立进程 + 稳定 IPC API | PRODUCT.md §12 D9 升级前置条件之一 |

以上均为 Later 候选，所有逻辑优先继续放入 daemon / mcp-server / desktop-shell 三个已落地包。`packages/desktop-shell/` 已从早期"候选"升级为正式 product layer 包，详 §3.2。
...
### ADR-8：desktop product layer 选 Electron（panel + pet + tray，frozen stack）

**决策**：Cairn product layer（`packages/desktop-shell/`）用 Electron（Node + Chromium）+ 原生 HTML/CSS/JS + better-sqlite3 实施，不选 Tauri 或 Native+WebView，不引前端框架（React / Vue / Svelte / Tailwind）也不引 TypeScript / Vite。
...
**后果**：
- Electron main process 直接 `require('better-sqlite3')` 读 `~/.cairn/cairn.db`（read-only），无独立 state-server 进程
- `packages/desktop-shell/` 是 product layer 唯一包，含 panel.html / pet (preview.html) / tray (in main.cjs) / legacy Inspector 四种 UI 形态
- electron-builder 打包；native module rebuild 用 `electron-rebuild`
- `package.json postinstall` 硬编码 `--target 32.3.3` 是已知 R15 风险（Electron 升级时需手动同步），README 已注明
- frozen stack：subagent verdict 2026-05-08 验证 Quick Slice 渲染量（≤200 row Run Log + ≤100 row Tasks）远在框架盈亏点之下；Activity Monitor / journalctl 这种"信息密度优先"审美与 utility-first CSS 框架冲突；TS + Vite 在 desktop-shell ~1500 LOC 之前是负 ROI。引入新 npm dep 需要重新评估

**未决**：
- 打包尺寸优化（ASAR、按需 Chromium 裁剪）留 Later
- macOS 公证 / Windows 签名留正式发布时处理
- tray icon 当前是 16×16 base64 PNG（idle/warn/alert 三档），hi-dpi 多分辨率 `.ico` 是 Hardening 项
...
#### 9c. `cairn install` CLI（W4 Phase 4）

> Disambiguation (2026-05-14): "Phase 4" in this section refers to the **kernel-layer** W4 milestone (processes / conflicts / dispatch + `cairn install` CLI, shipped 2026-05-06). It is **NOT** "desktop-shell Phase 4" (the Quick Slice product-layer milestone, shipped 2026-05-08 via the `f088b56..4c24fb6` commit chain). Whenever future docs say "Phase 4," qualify with "kernel" or "desktop-shell" — the same numbering across two different layers is a known historical naming collision (see project memory `cairn-dark-zones-and-pitfalls`).
